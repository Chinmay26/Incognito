...
class GraphConvert(object):
	...
	def convert(self, tf_graph_file_name, custom_objects=None):
		...
		tf_frozen_graph = self.freeze_session(K.get_session(), output_names=[out.op.name for out in keras_model.outputs])
		tf.train.write_graph(tf_frozen_graph,  base_dir, tf_graph_file_name, as_text=False)
